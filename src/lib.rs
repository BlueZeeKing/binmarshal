...
pub use binmarshal_macros::{Demarshal, Marshal, MarshalUpdate, Proxy};
...
impl<'dm, T: Demarshal<'dm, ()>, E: Demarshal<'dm, ()>> Demarshal<'dm, ()>
    for core::result::Result<T, E>
{
    fn read(view: &mut BitView<'dm>, _ctx: ()) -> Result<Self, MarshalError> {
        let tag = u8::read(view, ())?;
        match tag {
            0 => T::read(view, ()).map(Ok),
            _ => E::read(view, ()).map(Err),
        }
    }
}
...
impl<'dm, L: TryInto<usize> + DemarshalOwned> Demarshal<'dm, ()> for LengthTaggedPayloadOwned<L> {
    fn read(view: &mut BitView<'dm>, _: ()) -> Result<Self, MarshalError> {
        let l = L::read(view, ())?;
        let as_usize = l.try_into().map_err(|_| MarshalError::CoercionError)?;
        Ok(LengthTaggedPayloadOwned::new(
            view.take_aligned_slice(as_usize)?.to_vec(),
        ))
    }
}

impl<C: Clone, T: Marshal<C>, L: TryFrom<usize> + Marshal<()>> Marshal<C>
    for LengthTaggedSlice<L, T>
{
    fn write<W: BitWriter>(&self, writer: &mut W, ctx: C) -> Result<(), MarshalError> {
        match L::try_from(self.inner.len()) {
            Ok(v) => {
...
impl<'dm, C: Clone, T: Demarshal<'dm, C>, L: TryInto<usize> + DemarshalOwned> Demarshal<'dm, C>
    for LengthTaggedVec<L, T>
where
...
mod tests {
    ...
    fn test_u8() {
        ...
        assert_eq!(
            u8::read(&mut reader, BitSpecification::<1>),
        ...
        assert_eq!(
            0b1010_1101u8.write(&mut writer, BitSpecification::<8>),
            Ok(())
        );
