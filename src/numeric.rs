use crate::{rw::*, BitSpecification, Buffer, Demarshal, Marshal, MarshalError};

macro_rules! unsigned_numeric_impl {
    ( $t:ty ) => {
        impl<const BITS: usize> Marshal<BitSpecification<BITS>> for $t {
            #[inline(always)]
            fn write<W: BitWriter>(
                &self,
                writer: &mut W,
                _ctx: BitSpecification<BITS>,
            ) -> Result<(), MarshalError> {
                let offset = writer.bit_offset();
                if offset + BITS <= <$t>::BITS as usize {
                    let (arr, offset) =
                        writer.reserve_and_advance::<{ <$t>::BITS as usize / 8 }>(0, BITS)?;
                    let me = self << (<$t>::BITS as usize - (BITS + offset));
                    let me_bytes = me.to_be_bytes();

                    let mask = (<$t>::MAX >> (<$t>::BITS as usize - BITS))
                        << (<$t>::BITS as usize - (BITS + offset));
                    ...
                    let (arr, offset) =
                        writer.reserve_and_advance::<{ <$t>::BITS as usize / 8 + 1 }>(0, BITS)?;
                    ...
                    for i in 1..(<$t>::BITS as usize / 8) {
                        arr[i] = ((me_bytes[i - 1] & (u8::MAX >> (8 - first_bits)))
                            << (8 - first_bits))
                            | (me_bytes[i] >> (8 - first_bits));
                    }

                    arr[<$t>::BITS as usize / 8] &=
                        !((u8::MAX >> (8 - last_bits)) << (8 - last_bits));
                    arr[<$t>::BITS as usize / 8] |= (me_bytes[<$t>::BITS as usize / 8 - 1]
                        & (u8::MAX >> (8 - last_bits)))
                        << (8 - last_bits);
        ...
        impl<'dm, const BITS: usize> Demarshal<'dm, BitSpecification<BITS>> for $t {
            ...
            fn read(
                ...
                if offset + BITS <= <$t>::BITS as usize {
                    ...
                    Ok((v >> (<$t>::BITS as usize - (BITS + offset)))
                    ...
                    let first_n = unsafe {
                        &*(arr[0..(<$t>::BITS as usize / 8)].as_ptr() as *const [u8; {
                            <$t>::BITS as usize / 8
                        }])
                    };
