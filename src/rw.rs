...
impl<'a> BitView<'a> {
    ...
    #[inline(always)]
    pub fn catch_up(&mut self, other: &Self) {
        assert!(self.data == other.data);

        self.offset_bit = other.offset_bit;
        self.offset_byte = other.offset_byte;
    }
    ...
    pub fn check_magic<const N: usize>(&mut self, magic: &[u8; N]) -> Result<(), MarshalError> {
        ...
        match self.take::<N>(N, 0) {
            ...
            Ok(_) => Err(MarshalError::IllegalValue {
                byte_offset: self.offset_byte,
                bit_offset: self.offset_bit,
            }),
            Err(e) => Err(e),
        }
    }
    ...
    pub fn take<const N: usize>(
        ...
        bytes: usize,
        bits: usize,
    ) -> Result<(&'a [u8; N], usize), MarshalError> {
        ...
        let out = self
            .data
            .get(self.offset_byte..self.offset_byte + N)
            .map(|x| (unsafe { &*(x.as_ptr() as *const [_; N]) }, self.offset_bit));
    ...
    pub fn take_until(&mut self, sentinel: u8) -> Result<&'a [u8], MarshalError> {
        self.align(1);
        let end = (&self.data[self.offset_byte..])
            .into_iter()
...
pub trait BitWriter {
    ...
    fn reserve_and_advance<const N: usize>(
        &mut self,
        bytes: usize,
        bits: usize,
    ) -> Result<(&mut [u8; N], usize), MarshalError>;
    fn reserve_and_advance_aligned_slice(
        &mut self,
        bytes: usize,
    ) -> Result<&mut [u8], MarshalError>;
...
impl<'a> BitWriter for BufferBitWriter<'a> {
    ...
    fn reserve_and_advance<const N: usize>(
        ...
        let out = self
            .out
            .get_mut(self.offset_byte..self.offset_byte + N)
            .map(|x| {
                (
                    unsafe { &mut *(x.as_ptr() as *mut [_; N]) },
                    self.offset_bit,
                )
            });
